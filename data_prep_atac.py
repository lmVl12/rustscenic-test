...
# cells selected by RNA barcodes, then filter peaks by min_cells_pct, then select top variable peaks if >150k (just in case of large peak sets)
    ...
    if adata.n_vars > DATASETS[CURRENT_RUN]["qc"]["n_top_peaks"]:
        print("Large peak set detected. Selecting top variable peaks...")
        sc.pp.highly_variable_genes(adata, n_top_genes=DATASETS[CURRENT_RUN]["qc"]["n_top_peaks"], flavor="seurat_v3", check_values=False)
